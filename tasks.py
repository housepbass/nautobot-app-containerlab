...
# Variables may be overwritten in invoke.yml or by the environment variables INVOKE_CONTAINERLAB_xxx
...
def _is_compose_included(context, name):
    return f"docker-compose.{name}.yml" in context.containerlab.compose_files
...
def docker_compose(context, command, **kwargs):
    ...
    build_env = {
        ...
        "COMPOSE_HTTP_TIMEOUT": context.containerlab.compose_http_timeout,
        "NAUTOBOT_VER": context.containerlab.nautobot_ver,
        "PYTHON_VER": context.containerlab.python_ver,
        **kwargs.pop("env", {}),
    }
    compose_command_tokens = [
        "docker compose",
        f"--project-name {context.containerlab.project_name}",
        f'--project-directory "{context.containerlab.compose_dir}"',
    ]

    for compose_file in context.containerlab.compose_files:
        compose_file_path = os.path.join(context.containerlab.compose_dir, compose_file)
...
def build(context, force_rm=False, cache=True):
    ...
    print(f"Building Nautobot with Python {context.containerlab.python_ver}...")
...
def makemigrations(context, name=""):
    ...
    command = "nautobot-server makemigrations containerlab"
...
@task(
    help={
        "version": "Version of Containerlab to generate the release notes for.",
    }
)
...
def pylint(context):
    ...
    command = 'pylint --init-hook "import nautobot; nautobot.setup()" --rcfile pyproject.toml containerlab'
    run_command(context, command)
...
def unittest_coverage(context):
    ...
    command = "coverage report --skip-covered --include 'containerlab/*' --omit *migrations*"
...
def tests(context, failfast=False, keepdb=False, lint_only=False):
    ...
    if not is_truthy(context.containerlab.local):
